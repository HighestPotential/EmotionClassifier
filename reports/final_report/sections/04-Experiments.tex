...
Evaluating the EfficientNetV2-S model trained with standard sampling exposed a severe prediction bias toward the majority class. While achieving a global accuracy of 60.17\%, the network massively over-predicted \textit{happiness} (96.62\%), but almost entirely failed to recognize \textit{fear} (17.20\%) and \textit{disgust} (29.99\%). To address this disparity, we evaluated two advanced loss functions. Class-Balanced Focal Loss (CB-FL) assigns larger weights to underrepresented classes and dynamically scales the loss to focus on hard examples. Alternatively, Label-Distribution-Aware Margin Loss (LDAM) with Deferred Re-Weighting (DRW) enforces wider decision margins for minority classes and applies delayed frequency weighting. Implementing these techniques resulted in a revised global accuracy of XX\%, with the minority class accuracy for \textit{fear} improving to XX\% and \textit{disgust} reaching XX\%, ultimately yielding a more uniform performance distribution.
