...
def main():
    ...
    os.makedirs(os.path.join(args.output, "restored_imgs64"), exist_ok=True)
    os.makedirs(os.path.join(args.output, "cropped_faces64"), exist_ok=True)
    os.makedirs(os.path.join(args.output, "restored_faces64"), exist_ok=True)
    ...
    for img_path in img_list:
        ...
        if restored_img is not None:
            ...
            imwrite(
                restored_img,
                os.path.join(args.output, "restored_imgs64", out_name)
            )

        # ---------- SAVE FACES ----------
        cropped_faces = [cv2.resize(f, (64, 64)) for f in cropped_faces]
        restored_faces = [cv2.resize(f, (64, 64)) for f in restored_faces]

        for idx, (cf, rf) in enumerate(zip(cropped_faces, restored_faces)):
            imwrite(
                cf,
                os.path.join(args.output, "cropped_faces64", f"{basename}_{idx:02d}.png")
            )
            imwrite(
                rf,
                os.path.join(args.output, "restored_faces64", f"{basename}_{idx:02d}.png")
            )
