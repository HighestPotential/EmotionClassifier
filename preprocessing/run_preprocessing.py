...
if __name__ == "__main__":
    ...
    steps = [FaceRotationFilter(), FaceOrientationFilter(),\
        LightingFilter(), RemoveBlurredFaces(),\
             CroppingFace(), ResizingTo64(), ToRGB()]
    ...
    # CHANGED: Replaced Windows paths with Linux paths joined to the base directory
    input_path = os.path.join(base_path, "datasets")
    output_path = os.path.join(base_path, "preprocessed_dataset")

    print(f"Reading from: {input_path}") # CHANGED: Added print to verify path correctness during run
    
    dataset_runner.run_folder(pipeline, 
        input_path, 
        output_path, 
        keep_structure=True, max_files=None, log_every=100)
