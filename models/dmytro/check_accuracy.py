...
# Path for CCT
cct_path = os.path.join(os.getcwd(), 'models/dmytro/CCT-7withSAM')
if cct_path not in sys.path:
    sys.path.append(cct_path)
# Path for Vim
vim_path = os.path.join(os.getcwd(), 'models/dmytro/VIM/Vim')
...
def get_dataloader(data_path):
    ...
    if os.path.exists(data_path):
        ...
        # Pin memory for speed logic
        return torch.utils.data.DataLoader(ds, batch_size=BATCH_SIZE, shuffle=False, num_workers=8, pin_memory=True), len(ds)
    else:
        return None, 0
...
if __name__ == "__main__":
    ...
    datasets_to_check = ['AffectNet', 'CKplusIm', 'FERPlus', 'RAF-DB', 'KDEFFormated', 'jaffeFormated', 'MMAFEDB', 'NONAMEFormated', 'ExpWFormated', 'EmoSet-118k']
