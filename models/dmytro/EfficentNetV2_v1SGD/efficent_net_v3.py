...
ACCUM_STEPS = 1          
EPOCHS = 300
# SGD LEARNING RATE: 0.05 (V1 setting that reached 74%)
LR = 0.05
WEIGHT_DECAY = 5e-4      # Back to V1 setting
IMG_SIZE = 64
NUM_WORKERS = 8
...
DROP_PATH_RATE = 0.2
MIXUP_ALPHA = 0.2
CUTMIX_ALPHA = 0.1        # Restored to 0.1 (V1)
LABEL_SMOOTHING = 0.1     # Restored to 0.1 (V1)
WARMUP_EPOCHS = 5
...
transform_train_final = v2.Compose([
   ...
   # Photometric Transforms (Stackable)
    ...
    # RandomErasing (conservative: only 15% of images, tiny patches 2-10% area)
...
train_loader = torch.utils.data.DataLoader(full_train_dataset, batch_size=BATCH_SIZE, shuffle=False, num_workers=NUM_WORKERS, drop_last=True, pin_memory=True, sampler=sampler)
...
# SGD Optimizer
print(f"Using SGD with LR={LR} and WD={WEIGHT_DECAY}")
...
if __name__ == '__main__':
    ...
    if args.resume and os.path.isfile(args.resume):
        ...
        if 'model_ema_state_dict' in checkpoint:
            ...
            print("Warning: EMA state dict not found in checkpoint.")
            
        if 'optimizer_state_dict' in checkpoint:
            optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
        else:
            print("Warning: Optimizer state dict not found in checkpoint. Optimizer will restart.")
            
        if 'scheduler_state_dict' in checkpoint:
            scheduler.load_state_dict(checkpoint['scheduler_state_dict'])
        else:
            print("Warning: Scheduler state dict not found. Scheduler will restart.")

        start_epoch = checkpoint.get('epoch', 0)
        # Check if the saved epoch is 0-indexed or was next epoch
        # Usually checking current epoch
        
        best_val_acc = checkpoint.get('best_acc', 0.0)
        print(f"Resumed from epoch {start_epoch}, Best Acc: {best_val_acc:.2f}%")

    save_dir = '/home/d/dumanskyy/work/EmotionClassifier/models/dmytro/EfficentNetV2_v1SGD'
    os.makedirs(save_dir, exist_ok=True)

    # --- CSV Logging Setup ---
    csv_path = os.path.join(save_dir, 'efficientnetv3_sgd_restored_results.csv')
    # Build CSV header: Epoch, LR, Train Loss, Global Val Acc, + per-dataset columns
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        if val_acc > best_val_acc:
            ...
            }, os.path.join(save_dir, 'efficientnetv3_best.pth'))
        ...
        torch.save({
            ...
            'model_ema_state_dict': model_ema.module.state_dict(),
            'best_acc': best_val_acc
        }, os.path.join(save_dir, 'efficientnetv3_last.pth'))
