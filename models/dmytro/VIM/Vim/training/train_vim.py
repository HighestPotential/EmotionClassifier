...
# --- Save Directory ---
SAVE_DIR = os.path.join(vim_root, 'results')
...
val_loader = torch.utils.data.DataLoader(
    full_val_dataset, batch_size=BATCH_SIZE, shuffle=False,
    num_workers=NUM_WORKERS, pin_memory=True, persistent_workers=True
)

target_val_loaders = {}
for name, ds in val_datasets_map.items():
    target_val_loaders[name] = torch.utils.data.DataLoader(
...
if __name__ == '__main__':
    ...
    if args.dry_run:
        ...
        if len(full_train_dataset) > 0:
            test_loader = torch.utils.data.DataLoader(full_train_dataset, batch_size=2, shuffle=True)
    ...
    csv_path = os.path.join(SAVE_DIR, 'vim_results.csv')
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        if val_acc > best_val_acc:
            ...
            }, os.path.join(SAVE_DIR, 'vim_tiny_best.pth'))
        ...
        torch.save({
            ...
            'model_ema_state_dict': model_ema.module.state_dict(),
            'best_acc': best_val_acc
        }, os.path.join(SAVE_DIR, 'vim_tiny_last.pth'))
