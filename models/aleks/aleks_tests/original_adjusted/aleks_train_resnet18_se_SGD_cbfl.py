...
# Fix import path for model file in parent dir
...
def main():
    ...
    # Linux Paths
    ready_root = os.getenv("READY_ROOT", "/home/d/dumanskyy/work/EmotionClassifier/latest_3_0_ready_to_use_datasets")
    weights_dir = "/home/d/dumanskyy/work/EmotionClassifier/models/aleks/original_adjusted"
    ...
    best_eval = 0.0
    best_train = 0.0
    best_path = os.path.join(weights_dir, "best_resnet18_se_SGD_cbfl200.pth")
    checkpoint_path = os.path.join(weights_dir, "checkpoint_resnet18_se_SGD_cbfl200.pth")
    
    # CSV file setup
    csv_path = os.path.join(results_dir, "results_SGD_class_balanced_focal_loss200.csv")
    ...
    if args.resume and os.path.isfile(checkpoint_path):
        ...
        best_ep = ckpt.get('best_ep', 0)
        print(f"Resumed at epoch {start_epoch} (Best: {best_eval:.2f}%)")
        
    if args.dry_run:
        epochs = start_epoch + 1 if start_epoch < epochs else epochs
