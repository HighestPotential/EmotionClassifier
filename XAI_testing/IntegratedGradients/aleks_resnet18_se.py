...
class ResNet18SE(nn.Module):
    def __init__(self, num_classes=6, reduction=16):
        ...
        self.conv1 = nn.Conv2d(3, 64, 3, stride=1, padding=1, bias=False) # (64, 64) -> (64, 64)
        self.bn1 = nn.BatchNorm2d(64)
        self.relu = nn.ReLU(inplace=True)

        self.layer1 = self._make_layer(64, 2, stride=1, reduction=reduction)
        self.layer2 = self._make_layer(128, 2, stride=2, reduction=reduction) #(64, 64) -> (32, 32)
        self.layer3 = self._make_layer(256, 2, stride=2, reduction=reduction) #(32, 32) -> (16, 16)
        self.layer4 = self._make_layer(512, 2, stride=2, reduction=reduction) #(16, 16) -> (8, 8)
